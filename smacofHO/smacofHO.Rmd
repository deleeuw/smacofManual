...
If the $\Delta_i^j$ contain the zero vector, then the unconstrained minimum of \@ref(eq:snmu) iszero. Collapsing all $x_i$ and all $y_l^j$ into a single point makes all distances zero, and thus makes stress zero. 
...
w_{ij}&=\sum_{l=1}^{k_j}g_{il}^jw^j_{il}.(\#eq:wred)
\end{align}
...
(\#eq:matsnmu)
\end{equation}
The $W_j$ are diagonal matrices with the $w_{ij}$ from \@ref(eq:wred) 
on the diagonal.
...
Homogeneity analysis can be most naturally introduced using the concept of a *star plot*. For variable $j$ we plot $k_j$ category points and $n$ object
...
loss ... over the $\hat d_i^j$ is a monotone regression problem for a simple tree
...
2. The $Y_j$ are centroids, i.e. $Y_j=(G_j'W_jG_j)^{-1}G_j'W_jX$.
3. The $Y_j$ have rank one, i.e. $Y_j=q_ja_j'$.
...
constraint $X_j=X$). The smacof iterations, or Guttman transforms, more or less ignore the fact that we are dealing with a rectangular matrix and use the weights to transform the problem into a symmetric one (as in @heiser_deleeuw_A_79).
...
$$
We solve ... for $\tilde Y$ and then use $\tilde X=R_W^{-1}(P+W\tilde Y)$.
...
X&=\{W_\star\}^{-1}\sum_{j=1}^m\left\{R_j\tilde X_j-W_j(Y_j-\tilde Y_j)\right\}.(\#eq:stateq2)
\end{align}
