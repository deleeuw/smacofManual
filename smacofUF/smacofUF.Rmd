...
Our coding implies that 
$w_{il}^j\delta_{il}^j=g_{il}^j(1-g_{il}^j)=0$ for all i,j,l. Thus
$$
\sigma(X,Y_1,\cdots,Y_m)=\sum_{j=1}^m\sum_{i=1}^n\sum_{l=1}^{k_j}g_{il}^jd^2(x_i,y^j_l)
$$

Now let
$$
G=\begin{bmatrix}G_1&\cdots&G_m\end{bmatrix}
$$
and
$$
Z=\begin{bmatrix}
...
$$
$$
d^2(x_i,y_l^j)=\text{tr}\ Z'A_{il}^jZ
$$
# Examples 
