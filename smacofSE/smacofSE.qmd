...
The best way to avoid unwanted (i.e. non-global) local minima in smacof is to start the iterations with an excellent initial configuration Even if the iterations converge to a non-global local minimum, at least we can be sure than the solution has a lower stress
than the initial configuration, which was presumably already excellent.
...
Classical Scaling, as an MDS technique, has one enormous advantage over other forms
...
$$
with $D^{(2)}(X)$ the squared Euclidean distances. This was first pointed out
by @deleeuw_heiser_C_82. Not only does Classical Scaling minimize strain,
...
has only $q$ non-zero dimensions (and $p-q$ zero dimensions). This disadvantage
is usually not a very serious one, because having fewer than $p$ positive
eigenvalues suggests a horrendously bad fit so that maybe MDS is not a good
idea in the first place. Alternatively, and more optimistically, one can
compute an additive constant to force positive semi-definiteness (@cailliez_83).
