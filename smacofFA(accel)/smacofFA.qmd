...
$$
$$
V^+B(C+\Xi)(C+\Xi)B(C+\Xi)V^+
$$

$$
V^+B(C)C\mathcal{D}B(C)(\Xi)V^++V^+\mathcal{D}B(C)(\Xi)CB(C)V^++V^+B(C)\Xi B(C)V^+
$$
If $V^+B(C)C=C$ then 
$$
C\mathcal{D}B(C)(\Xi)V^++V^+\mathcal{D}B(C)(\Xi)C+V^+B(C)\Xi B(C)V^+
$$
...
$$
d_{ij}^{-1}(C+\Xi)=d_{ij}^{-1}(C)-\frac12\frac{\text{tr}\ A_{ij}\Xi}{d_{ij}(C)}
$$
$$
B(C+\Xi)=B(C)-\frac12\sum_{i<j}w_{ij}\frac{\delta_{ij}}{d_{ij}(C)}\text{tr}\ A_{ij}\Xi A_{ij}
$$

$$
\mathcal{D}B(C)(\Xi)=-\frac12\sum_{i<j}w_{ij}\frac{\delta_{ij}}{d_{ij}(C)}(\xi_{ii}+\xi_{jj}-2\xi_{ij}) A_{ij}
$$
...
```{r code2, code = readLines("smacofDerivatives.R"), eval = FALSE}
```

## smacofPCADerivative.R

```{r code3, code = readLines("smacofPCADerivative.R"), eval = FALSE}
```

## smacofQRDerivative.R

```{r code4, code = readLines("smacofQRDerivative.R"), eval = FALSE}
```

## smacofCompare.R

```{r code5, code = readLines("smacofCompare.R"), eval = FALSE}
```

## smacofUtils.R

```{r code6, code = readLines("smacofUtils.R"), eval = FALSE}
```
