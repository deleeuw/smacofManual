...
\begin{proof}
The first part of the theorem merely states the obvious. If
\(f(x)=h(x^2)\) with \(h\) concave then \(h'\) is decreasing, which
means \(h'(x^2)\) is decreasing on \(\mathbb{R}^+\) ad thus \(f'(x)/x\)
is decreasing..
\end{proof}
...
\] For all \(y^2<1\) the majorizer \(g\) has its minimum at zero. If
\(y^2>1\) the majorizer is a concave quadratic, which has no minimum and
is unbounded below. If \(y^2=1\) the majorizer is the horizontal line
\(x=0.25\) and the majorization method stops at a local maximum. This is
illustrated in Figure~\ref{fig-quartic1} and Figure~\ref{fig-quartic2}.
...
\pandocbounded{\includegraphics[keepaspectratio]{av_files/figure-pdf/fig-quartic1-1.pdf}}

}

\caption{\label{fig-quartic1}A Quartic, Majorized at \(\pm 0.5\)}
...
\pandocbounded{\includegraphics[keepaspectratio]{av_files/figure-pdf/fig-quartic2-1.pdf}}

}

\caption{\label{fig-quartic2}A Quartic, Majorized at \(\pm 1.25\)}
...
of LS and LAV then that leads us to Equation~\ref{eq-huberb}.
...
We start with a LS analysis, actually with Huber loss with \(c=10\),
which for these data is equivalent to LS. The process converges in 859
iterations.
...
For LAV we use Chardonnier loss with \(c=.001\). We have convergence in
2291 iterations.
...
(\citeproc{ref-weiszfeld_37}{1937})). His algorithm avant-la-lettre can
...
and Zamir (\citeproc{ref-gabriel_zamir_79}{1979}). The alternating is to
...
\begin{center}
\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{av_files/figure-pdf/change-1.pdf}
\end{center}
...
\begin{center}
\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{av_files/figure-pdf/traject-1.pdf}
\end{center}
