...
This case has been studied in detail (in the case of real-valued functions on the line) by @vanruitenburg_05 and @deleeuw_lange_A_09. Their Theorem 4.5 on page 2478 says
...
@deleeuw_lange_A_09 give some references to previous work on results like  @thm-wght, notably @groenen_giaquinto_kiers_03, @jaakola_jordan_00, and @hunter_li_05. In these earlier papers we do not find @thm-wght in its full
generality, mostly because the authors have specific applications in mind,
which they then embed in a likelihood, Bayesian, or EM framework. There are
similar appearances of results very much like @thm-wght and @thm-sqrt in location analysis,
image restoration @geman_reynolds_92. In robust statistics it has been known
that iterative weighted least squares with weights $f'(x)/x$ gives a quadratic
majorization algorithm.

We now apply theorem ... to functions of the form
\begin{equation}
...
Although the absolute value is not differentiable at the origin the theorem can still be applied (@vanruitenburg_05). It just does not give a majorizer at $y=0$. (@vanruitenburg_05)
If $f(x)=|x|$ then
\begin{equation}
...
@ramirez_sanchez_kreinovich_argaez_14 argue \eqref{eq:charf} is also the "most computationally efficient smooth approximation to $|x|$".
...
```{r gcharfig, fig.align = "center", echo = FALSE, fig.cap = "Generalized Charbonnier Loss", fig.dim = c(6, 6)}
par(pty = "s")
...
```{r welshfig, fig.align = "center", echo = FALSE, fig.cap = "Welsch Loss", fig.dim = c(6, 6)}
par(pty = "s")
