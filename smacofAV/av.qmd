...
\end{equation}
This modified $\omega_1$ majorizes $\sigma_1$ at $Y$, even in the perverse case that $\delta_k=d_k(Y)$ for all $k$. But ...
...
To fix the terminology we say that a function $g$
...
\end{equation}
It then recomputes the weights $w_k(X^{(k+1)})$ and goes to the smacof step again. This can be thought of as iteratively reweighted least squares, and also as majorization within majorization, with the smacof majorization within the sharp quadratic majorization of the loss function.
...
@deleeuw_E_18f, @ramirez_sanchez_kreinovich_argaez_14
