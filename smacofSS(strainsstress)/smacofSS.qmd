...
$${#eq-cstrain}
Minimizing @eq-cstrain over $p$-dimensional configurations is indeed classical scaling. The matrix form in @eq-strain strain was first given
...
Suppose $V$ is a symmetric and doubly-centered positive semi-definite matrix
of rank $n-1$. Define strain as
$$
\sigma(X):=\frac14\text{tr}\ V^2(\Delta^2-D^2(X))V^2(\Delta^2-D^2(X)).
$${#eq-vstrain}
The choice of $V$ defines a number of homogeneous linear combinations
of the residuals $\Delta^2-D^2(X)$ whose squares we minimize. We can choose
$V$ to be a banded matrix, a block matrix with diagonal blocks, and so on.
If $V=J$ we are back at @eq-strain. Note that $J$ is the only idempotent 
doubly-centered positive semi-definite matrix of rank $n-1$.

With @eq-vstrain the global optimization advantage of classical scaling is still maintained.
Suppose $s_i=x_i'x_i$. Then $D^2(X)=se'+es'-2XX'$. Thus with $B:=-\frac12V\Delta^2V$
...
$VX$ from $B$, and set $X=V^+K\Lambda L'$, with $K$ and $\Lambda$ from the eigen-analysis
of $B$ and $L$ an arbitrary rotation. For non-Euclidean $\Delta^2$ different $V$ will give different solutions.
...
$$
\text{tr}\ (B(\theta)-VXX'V)^2
$${#eq-torgcomp1}
over $VX$ for fixed $\theta$ is classical MDS. Minimizing 
$$
\text{tr}\ ((B_0-VXX'V)-\mathop{\sum\sum}_{1\leq i<j\leq n}\theta_{ij}T_{ij})^2
$${#eq-torgcomp2}
...
We alternate the two subproblems in an outer iteration. Both eigs-sym and nnls have a cold start and iterate until convergence, which may be wasteful in later outer iterations. We may eventually replace them with the hot start majorization methods in smacofEigenRoutines.R and smacofNNLS.R (both in the smacofUtilities directory in smacofCode).
