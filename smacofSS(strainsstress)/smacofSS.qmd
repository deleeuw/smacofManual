...
For the first problem we use the eigs_sym function from the RSpectra package (@qiu_mei_24), for the second the quadprog package and function (@turlach_weingessel_19). Both functions in an inner iteration have a 
cold start, which may be wasteful in later iterations. The plan therefor is to eventually replace them with the majorization methods in smacofEigenRoutines.R
