...
\date{Started February 21 2024, Version of April 17, 2024}
...
There is also something called \emph{non-metric MDS}. The terms is ambiguous and can mean either one of two things. One definition is that
...
to use only the ordinal or nominal information as data in out MDS
analysis. Thus our approximation problem is of the form
\begin{equation}
\alpha\{(o_i,o_j),(o_k,o_l)\}\approx \beta(d_{ij}(X),d_{kl}(X)),
\label{eq:mdsnonmetricformdef}
\end{equation}
where \(\alpha\) and \(\beta\) are appropriate binary relations.

The term ``non-metric'' is also used in the case in which we do have numerical dissmilarities \(\delta_{ij}\), but the MDS problem is to find Euclidean distances which approximate some partially unknown function of the dissimilarities. The function should be member of a well-defined class of functions, for
instance a third degree polynomial or a piecewise linear spline on a
given knot sequence. The MDS techniquw not only find the map of
...
\label{eq:mdsnonlineardef}
\end{equation}
Note that \eqref{eq:mdsnonlineardef} implies that \(f\) is applied to each
...
One radical way to solve this terminological dilemma is to simply define
MDS as solving
\begin{subequations}
\begin{align}
D(X)&\approx\Delta,\label{eq:gmdsdef1}\\
\Delta&\in\mathfrak{D},\label{eq:gmdsdef2}
\end{align}
\end{subequations}
where \(D(X)\) is the matrix of Euclidean distances and \(\mathfrak{D}\) is a set of \emph{transformed dissimilarities} in the non-linear case or a set of \emph{quantified dissimilarities} in the non-metric case. We can then freely discuss
metric MDS, linear MDS, non-linear MDS, and non-metric MDS as special cases,
defined by different sets \(\mathfrak{D}\).

\section{Kruskal's Stress}\label{kruskals-stress}
...
dissimilarities by the distances. To be historically accurate, we should
also mention that the non-metric MDS technique proposed by Shepard (1962a) and Shepard (1962b) can be reformulated as minimization of an explicit loss function (see, for example, De Leeuw (2017b)). In fact the classical Young-Householder-Torgerson MDS technique (Torgerson (1952)) for metric MDS also can be formulated as minimizing an explicit least squares loss function (De Leeuw and Heiser (1982)).
...
\emph{dimension} of the Euclidean space in which we make the map. The metric MDS problem (of dimension \(p\), for given \(W\)
...
\subsection{Non-linear and Non-metricMDS}\label{non-linear-and-non-metricmds}
