...
There is also something called *non-metric MDS*. The terms is ambiguous and can mean either one of two things. One definition is that
...
(\#eq:mdsnonlineardef)
\end{equation}
Note that \@ref(eq:mdsnonlineardef) implies that $f$ is applied to each 
...
\begin{subequations}
\begin{align}
D(X)&\approx\Delta,(\#eq:gmdsdef1)\\
\Delta&\in\mathfrak{D},(\#eq:gmdsdef2)
\end{align}
\end{subequations}
where $D(X)$ is the matrix of Euclidean distances and $\mathfrak{D}$ is a set of *transformed dissimilarities* in the non-linear case or a set of *quantified dissimilarities* in the non-metric case. We can then freely discuss
...
also mention that the non-metric MDS technique proposed by @shepard_62a and @shepard_62b can be reformulated as minimization of an explicit loss function (see, for example, @deleeuw_E_17e). In fact the classical Young-Householder-Torgerson MDS technique (@torgerson_52) for metric MDS also can be formulated as minimizing an explicit least squares loss function (@deleeuw_heiser_C_82).
...
*dimension* of the Euclidean space in which we make the map. The metric MDS problem (of dimension $p$, for given $W$
